...
class SimLancher(Node):
    def __init__(self):
        ...
        python_script = os.path.join(
                    get_package_share_directory('isaac_diffbot_sim_scripts'), 'isaac_python.sh')
        start_script = os.path.join(
                    get_package_share_directory('isaac_diffbot_sim_scripts'), 'start_sim.py')
        command = ["bash", python_script, start_script, 
                    "-p", "./install/diffbot_description/share/diffbot_description/robots/diffbot.urdf"]
        print(command)
